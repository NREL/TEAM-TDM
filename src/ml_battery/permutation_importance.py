...
def get_score_importances(
    ...
    base_score = score_func(X, y, sample_weight=sample_weight)
    scores_decreases = []
    for i in range(n_iter):
        scores_shuffled = _get_scores_shuffled(
            score_func, X, y, sample_weight=sample_weight, columns_to_shuffle=columns_to_shuffle,
            random_state=rng
        )
...
def _get_scores_shuffled(score_func, X, y, sample_weight=None, columns_to_shuffle=None,
                        random_state=None):
    Xs = iter_shuffled(X, columns_to_shuffle, random_state=random_state)
    scores = []
    for X_shuffled in Xs:
        scores.append(score_func(X_shuffled, y, sample_weight=sample_weight))
